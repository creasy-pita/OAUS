    ...
    public class Updater
    {
        ...
        private void GetUpdateInfo(out IList<string> downLoadFileNameList, out IList<FileUnit> removeFileNameList)
        {
            ...
            if (this.updateConfiguration.ClientVersion != lastUpdateTimeContract.ClientVersion)
            {
                ...
                //**
                RemoveNotCurrentUserFile(contract.AllFileInfoList);
                //**
        ...
        //**
        /// <summary>
        /// 去除服务器端的路径与 客户端的文件名存在路径差异，
        /// </summary>
        /// <param name="fileList"></param>
        private void RemoveNotCurrentUserFile(IList<FileUnit> fileList)
        {
            for(int i=fileList.Count-1;i>=0;i--)
            {
                if(fileList[i].FileRelativePath.IndexOf(ClientID+"\\") <0 )
                {
                    fileList.RemoveAt(i);
                }
            }
